...
// Resolve a Chromium executable in a portable way:
//   1. Explicit override via PUPPETEER_EXECUTABLE_PATH
//   2. Reuse the Chrome for Testing build that Playwright already installed
//      (avoids a second ~150MB download and works on macOS/Linux/CI)
// @sparticuz/chromium was removed: it is an Amazon-Linux build tuned for AWS
// Lambda and cannot launch on a normal desktop or on Vercel's runtime.
...
// Images are referenced by their remote URL (not inlined as base64). Inlining
// every product image as a data URL produced a single HTML string that exceeded
// V8's maximum string length (RangeError) for catalogs of even modest size.
...
async function catalogTemplate(images, opts, _colors, _brand) {
  ...
  for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
    ...
    const cardPromises = page.map(async (img) => {
      const metaParts = [];
    ...
    const cards = (await Promise.all(cardPromises)).join('\n');
    renderedPages.push(`
      <div class="pdf-page">
  ...
  return { pageCards: renderedPages.join('\n'), totalPages: pages.length };
}

// ── Template: Line Sheet ────────────────────────────────────────────────────
async function lineSheetTemplate(images, opts) {
  ...
  for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
    const page = pages[pageIdx];
    const rowPromises = page.map(async (img) => {
      return `
          <div class="line-row">
            <div class="line-img">${imgTag(img.url, img.name)}</div>
...
// ── Template: Lookbook ──────────────────────────────────────────────────────
async function lookbookTemplate(images, opts) {
    ...
    const cardPromises = page.map(async (img) => {
      return `
          <div class="lookbook-card">
            <div class="lookbook-image">${imgTag(img.url, img.name)}</div>
...
// ── Build HTML ──────────────────────────────────────────────────────────────
async function buildHTML(images, opts = {}) {
  ...
  if (template === 'line-sheet') {
    templateResult = await lineSheetTemplate(images, opts);
  } else if (template === 'lookbook') {
    templateResult = await lookbookTemplate(images, opts);
  } else {
    templateResult = await catalogTemplate(images, opts, colors, brand);
  }
...
async function generatePDF(images, outputPath, opts = {}) {
  ...
  const html = await buildHTML(images, opts);
  ...
  if (!executablePath) {
    throw new Error(
      'No Chromium found. Set PUPPETEER_EXECUTABLE_PATH or install one (e.g. `npx playwright install chromium`).'
    );
  }
